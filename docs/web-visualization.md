...
3. Q-criterion 点群
   ...
   - `Q > 0` のうち強い領域を点群表示する。
4. 渦度 `|ω|` 等値面
   - 「強い領域の表示割合」から決まる渦度閾値を iso-value として使う。
   - Marching Tetrahedra で三角形メッシュを抽出する。
5. Q-criterion 等値面
   - `Q > 0` の母集団から閾値を決め、同じく三角形メッシュを抽出する。

「強い領域の表示割合」は、点群モードでは上位何%の格子点を表示するか、等値面モードではその上位領域の境界となる iso-value を決める。等値面では「間引き」を大きくするとセルを粗く辿るため軽くなる代わりに形状解像度が下がる。

ブラウザ側の速度勾配・渦度・Q-criterion は可視化用の軽量診断であり、研究用の定量値はソルバ側の Fourier 微分や保存済み診断量を優先する。また、等値面抽出は表示領域の末端セルで周期境界を跨がないため、周期箱の境界で面が切れて見える場合がある。
