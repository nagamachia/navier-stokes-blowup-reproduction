...
int main(int argc,char**argv){
    ...
    out<<"rho,radial_order,eta_order,product_bound,DX_bound,I_bound,J1_bound,J2_bound,deta_bound,"
          ...
          "sigma_star,analytic_radius,pole_distance,cauchy_factor,log_C_for_g,"
          "chi_norm,zeta_norm,invL_norm,Wstar_norm,Hstar_norm,"
    ...
    for(double rho:{1e-5,2e-5,5e-5,1e-4,2e-4}){
        ...
        const auto b=appendix_b_remainder_lipschitz_budget(
        ...
        out<<rho<<','<<op.radial_order<<','<<op.eta_order<<','<<op.product<<','<<op.DX<<','<<op.I<<','<<op.J1<<','<<op.J2<<','<<op.deta<<','
           ...
           <<sep.sigma_star<<','<<mult.outer_radius<<','<<mult.nearest_chi_zeta_pole_distance<<','
           <<mult.scalar_cauchy_factor<<','<<mult.log_C_for_g<<','
           <<mult.norms.chi<<','<<mult.norms.zeta<<','<<mult.norms.invL<<','<<mult.norms.Wstar<<','<<mult.norms.Hstar<<','
