...
int main() {
    ...
    if (!(log_contraction_proxy < 0.0)) {
        std::cerr << "finite B-rho naive remainder budget did not close\n"
                  << " logLambda=" << scale.log_Lambda_for_U_tolerance
                  << " logM=" << std::log(budget.M)
                  << " logProxy=" << log_contraction_proxy
                  << " sigma=" << sep.sigma_star
                  << " zetaNorm=" << mult.norms.zeta
                  << " product=" << brho.product
                  << " deta=" << brho.deta
                  << " mixedJ1=" << brho.mixed_J1
                  << " mixedJ2=" << brho.mixed_J2 << '\n'
                  << " R1={" << budget.r1_bracket_phi << ',' << budget.r1_W_DXPhi << ','
                  << budget.r1_H_detaPhi << "} R2={" << budget.r2_linear_u << ','
                  << budget.r2_u2 << ',' << budget.r2_W_DXu << ',' << budget.r2_H_deta_u << ','
                  << budget.r2_u_deta_u << ',' << budget.r2_pressure_p << ','
                  << budget.r2_pressure_peta << ',' << budget.r2_pressure_DXp << "}\n";
        return 11;
    }
    return 0;
}
