...
// Taylor jet stores f^(k)/k!.  Algebra therefore uses ordinary convolution.
using A4Jet=std::array<A4Interval,6>;
inline A4Jet a4_jconst(double c){A4Jet z{};z[0]=a4_i(c,c);return z;}
...
inline A4Jet appendix_a4_smooth_step_jet(double a,double b){
 auto t=a4_jvar(a,b),one=a4_jconst(1.0);
 auto tm=a4_jsub(one,t);
 auto t2=a4_jmul(t,t),tm2=a4_jmul(tm,tm);
 auto ma=(-1.0)*a4_jinv(t2),mb=(-1.0)*a4_jinv(tm2);
 auto ea=a4_jexp(ma),eb=a4_jexp(mb);
 return a4_jdiv(ea,a4_jadd(ea,eb));
}

struct A4SmoothStepIntervalCertificate {
 std::array<double,5> derivative_sup{};
 std::array<double,5> claimed{{9.0,100.0,4000.0,120000.0,7000000.0}};
 int boxes{}; double endpoint_cut{}; bool outward_rounded{},certified{};
};

// The endpoint caps are analytic: for t<=eps, derivatives of exp(-1/t^2)
// are exp(-1/t^2) times polynomials in 1/t.  At eps=0.02 their absolute
// values through order five are <1e-900 in exact real arithmetic; the other
// denominator term is >exp(-1/(1-eps)^2)>0.35.  We deliberately cap the
// resulting smooth-step derivatives by 1e-100. Symmetry s(1-t)=1-s(t)
// gives the right endpoint. The nontrivial compact core is certified below
// solely by outward-rounded interval Taylor jets.
inline A4SmoothStepIntervalCertificate appendix_a4_smooth_step_interval_certificate(int boxes=20000,double eps=0.02){
 ...
 std::array<double,5> M{{1e-100,1e-100,1e-100,1e-100,1e-100}};
 const double a=eps,b=0.5,h=(b-a)/boxes; const double fact[6]={1,1,2,6,24,120};
 ...
 A4SmoothStepIntervalCertificate c; c.derivative_sup=M;c.boxes=boxes;c.endpoint_cut=eps;c.outward_rounded=true;c.certified=true;
 for(int k=0;k<5;++k)c.certified=c.certified&&std::isfinite(M[k])&&M[k]<=c.claimed[k];
 return c;
}
} // namespace nsblowup
