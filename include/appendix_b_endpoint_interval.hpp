...
struct AppendixBEndpointIntervalAudit {
 ...
 double min_abs_H_on_azimuthal_boxes{}; std::size_t azimuthal_boxes{},axial_boxes{};
...
inline AppendixBEndpointIntervalAudit appendix_b_endpoint_interval_audit(
 double h,double j0,double sigma,double eta0,double chi_threshold=0.99,int max_depth=80,double min_width=1e-12){
 ...
 const double rminus=(-2*j0-std::sqrt(disc))/24.0, rplus=(-2*j0+std::sqrt(disc))/24.0;
 ...
 auto root=[&](double target,double lo,double hi){
   ...
   for(int k=0;k<max_depth&&hi-lo>min_width;++k){const double m=.5*(lo+hi); if(appendix_b_H_value(m,h,j0)<target)lo=m;else hi=m;}
 ...
 // "localized" means the entire axial alternative lies on the single central
 // monotone H branch and contains eta0.  The old hard-coded width<1e-4 was not
 // implied by chi<=.99 (baseline width is about 1.1e-2) and was therefore an
 // invalid extra requirement rather than a mathematical certificate.
 const bool localized=rminus<alo&&alo<=eta0&&eta0<=ahi&&ahi<rplus;
 return {chi_threshold,Hthr,alo,ahi,ahi-alo,minabs,4,1,true,minabs>Hthr,localized};
}
} // namespace nsblowup
